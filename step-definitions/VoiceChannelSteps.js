  ...
  async (callData) => {
    ...
    for (let i = 0; i < phoneList.length; i++) {
      let phoneNumber = await (
        await voiceChannel.getPhoneNumber(i)
      ).replace(/\s+/g, '');
...
Then('user dial {string} phone', async(number)=>{
  await voiceChannel.dialNumber(number);
});

Then('verify invalid number error is displayed', async()=>{
  await voiceChannel.verifyInvalidNumberError();
});

Then('verify {string} is triggered by the dialer', async(number)=>{
  await voiceChannel.verifyPowerDialNumber(number);
});