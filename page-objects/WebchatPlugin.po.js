...
exports.WebchatPlugin = class WebchatPlugin extends BaseAction {
  ...
  elements = {
    ...
    triggerAgentNameCheck: '//*[@data-translate="webchat_triggers_display_name_check"]/preceding-sibling::i',
    triggerSenderNameInput: '#wc-trigger-display',
    triggerMessageInput: '[name="wc-trigger-message"]',
    triggerSaveButton: '#wc-trigger-save',
    webchatPluginSender: '(//*[@data-type="MSG" and @data-from="SYSTEM"])[last()]//span[@class="name"]',
    webChatPluginMesg: '(//*[@data-type="MSG" and @data-from="SYSTEM"])[last()]/div/div[last()]',
  ...
  async triggerDetails(triggerData) {
    ...
    if (triggerData.when) {
      ...
      await this.selectOptionByText(this.elements.triggerWhenDropdown, triggerData.when);
    }
    if (triggerData.action) {
      await this.waitForSelector(this.elements.triggerActionDropdown);
      await this.selectOptionByText(this.elements.triggerActionDropdown, triggerData.action);
    }
    ...
    if (triggerData.displayName) {
      if (await this.checkCheckbosIsChecked(this.elements.triggerAgentNameCheck)){
  ...
  /**
   * function to reset trigger action
   * @returns {void} nothing
   */
  async resetTriggerValue() {
    global.triggerName = [];
  }
};
