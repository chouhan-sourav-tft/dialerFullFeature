...
exports.VoiceChannel = class VoiceChannel extends BaseAction {
  ...
  elements = {
    ...
    phoneNumber:
      '#dialer-control-hopper-preview-table>tbody>tr>td:nth-child(6)',
    ...
    deleteRecycle: '.recycle-rule-remove-btn',
    deleteCampaign: '#campaignList .btn-danger',
    campaignConfirm: '#delete-owner-btn',
    ...
    outboundCloudSubmit: '#voice-login-submit',
    ...
    invalidNumberError: '//div[@id="newContentdiv"]//p[contains(text(),"The dialed number does not exist.")]'
  };
  ...
  async createCampaign(settings) {
    ...
    if (!check) {
      ...
      if (settings.recycle ==='delete') {
        if(await this.isVisible(this.elements.deleteRecycle)){
          const totalRecycle = await this.countElement(this.elements.deleteRecycle);
          for(let i = 0; i < totalRecycle; i++){
  ...
  */
   ...
   * @return {int} phoneIndex - Phone number index
   */
  async getPhoneNumber(phoneIndex) {
    await this.wait(5);//wait to load phone
    let displayNumber = '';
    let phoneLocator = '';
    let phoneTextLocator = '';
    if(phoneIndex === 0){
      phoneLocator = '[id="voice-field-first_phone-btn"]';
      phoneTextLocator = '#voice-field-first_phone';
    }
    if(phoneIndex === 1){
      phoneLocator = '[id="voice-field-second_phone-btn"]';
      phoneTextLocator = '#voice-field-second_phone';
    }

    if(await this.isVisible(phoneLocator)){
      await this.waitForSelector(phoneLocator);
      await this.shouldVisible(phoneLocator);
      displayNumber = await this.getText(phoneLocator);
    }
    if(await this.isVisible(phoneTextLocator)){
      await this.waitForSelector(phoneTextLocator);
      await this.shouldVisible(phoneTextLocator);
      displayNumber = await this.getText(phoneTextLocator);
    }
   ...
   * @param {string} number - phone index
   * @param {string} type - session type
   * @returns {void} nothing 
   */
  async dialNumber(number, type=''){
    // waiting to enable call button
    await this.wait(4);
    let phoneLocator = `#voice-field-${number}_phone-btn`;
    await this.waitForSelector(phoneLocator, type);
    await this.click(phoneLocator, type);
    // waiting for call to connect
    await this.wait(3);
  }

  /** function to verify error message for invalid phone number
   * @returns {void} nothing 
   */
  async verifyInvalidNumberError() {
    await this.waitForSelector(this.elements.invalidNumberError);
    let popup = await this.isVisible(this.elements.invalidNumberError);
    assert.isTrue(popup);
  }

  /** function to verify triggered number
   * @param {string} number - phone number
   * @returns {void} nothing 
   */
  async verifyPowerDialNumber(number){
    await this.shouldContainText(this.elements.outboundCallNumberText, number);
  }
};
