...
exports.Database = class Database extends BaseAction {
  ...
  async createDatabase(databaseName, databaseDetails) {
    ...
    if (databaseDetails.phone) {
      let phoneArr = databaseDetails.phone.split(',');
      let phoneIDArr = databaseDetails.phoneID.split(',');
      for (let i = 0; i < phoneArr.length; i++) {
        let selector = `//select[@id="${phoneArr[i]}_phone"]`;
        await this.dropdownOptionSelect(selector, phoneIDArr[i]);
      }
    }
